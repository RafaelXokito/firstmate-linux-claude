...
- `FORGE_UNKNOWN: <reason>` - a registered project's origin host maps to no known forge, so firstmate cannot tell which delivery CLI and which authentication that project needs.
  Never treat it as "no forge": the report exists precisely because guessing that would drop a real gate for a self-hosted instance.
  Read the project's origin, then add one `<host> <github|gitlab|bitbucket|none>` line to `config/forge-map` so the requirement and the auth probe follow reality, and use `none` only for a project that genuinely ships without a forge.
  `bin/fm-forge-lib.sh` owns the resolution order.
